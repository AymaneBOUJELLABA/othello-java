...
public class BoardPanel extends javax.swing.JPanel {
    ...
    // Construction qui permet de initialiser le board a partir d'une board predefinis
    ...
    // Fonction qu'on appel pour dessiner 
    private void doDrawing(Graphics g) {
        Graphics2D g2d = (Graphics2D) g;

        // Dessiner l'arriere plan vert
        g2d.setColor(new Color(32, 144, 103));
        g2d.fillRect(0, 0, getWidth(), getHeight());

        // Dessin du partie du score
    ...
    // Fonction qui permet a le joueur (humain) de jouer dans une case.
    ...
    // Permet de passer a l'autre tourne
    public void switchTurn() {
        turn = turn == CaseValue.BLACK ? CaseValue.WHITE : CaseValue.BLACK;
    }

    // Permet d'affectuer le "capture" des pieces pour une case
    ...
    // Verifier si la partie est termine    
    ...
    // Calculer les cases disponible pour le joueur
    ...
    // Verifier la disponibilie d'une case
    private boolean checkCase(int i, int j, CaseValue turn) {
        // On boucle sur les 8 directions ( diagonales plus les lignes)
        for (int k = -1; k <= 1; k++) {
            for (int m = -1; m <= 1; m++) {
                if (k == 0 && m == 0) {
                    continue;
                }
                // On verfie si la direction est valable
    ...
    // Verfie la direction permet de la case a etre disp
    private boolean checkDir(int row, int col, int dRow, int dCol, CaseValue turn) {
        boolean foundOther = false;

        // On parcours la direction
        for (int i = row + dRow, j = col + dCol; i < 8 && i >= 0 && j < 8 && j >= 0; i += dRow, j += dCol) {
            /*
            Une direction est valide si:
                - Il y'a pas de case vide
                - Il existe une case d'autre joueur entre mes pieces
             */
    ...
    // Verifie si elle exsite une piece d'autre joueur adjacent a cette case
    private boolean adjacentToEnemy(int i, int j, CaseValue turn) {
        for (int k = -1; k <= 1; k++) {
            for (int m = -1; m <= 1; m++) {
                if (m == 0 && k == 0 || !isSafe(i + k, j + m)) {
                    continue;
                }
